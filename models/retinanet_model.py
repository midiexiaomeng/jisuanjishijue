...
class RetinaNetDetector:
    ...
    def _init_model(self):
        ...
        try:
            ...
            print(f"RetinaNet模型初始化完成，使用设备: {self.device}")
            
        except Exception as e:
            print(f"初始化RetinaNet模型失败: {e}")
            # 回退到简单的模型
            self._init_simple_model()
    
    def _init_simple_model(self):
        """初始化简单的RetinaNet模型（回退方案）"""
        try:
            # 加载预训练的RetinaNet模型
            ...
            self.model.head.classification_head.cls_logits = nn.Conv2d(
                in_channels, num_anchors * self.num_classes, kernel_size=3, stride=1, padding=1
            )
            
            # 移动到设备
            self.model.to(self.device)
            
            print(f"简单RetinaNet模型初始化完成，使用设备: {self.device}")
            
        except Exception as e:
            print(f"初始化简单RetinaNet模型失败: {e}")
            raise
    ...
    def train(self, train_loader=None, val_loader=None, epochs: int = 10, 
              learning_rate: float = 0.001, save_dir: str = 'checkpoints/retinanet',
              data_path: str = None, batch_size: int = 8, num_workers: int = 2):
        """
        训练模型
        ...
        print(f"开始训练RetinaNet模型，共{epochs}轮...")
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 5 == 0 or epoch == epochs - 1:
                checkpoint_path = os.path.join(save_dir, f'retinanet_epoch_{epoch+1}.pth')
                torch.save({
                    'epoch': epoch + 1,
